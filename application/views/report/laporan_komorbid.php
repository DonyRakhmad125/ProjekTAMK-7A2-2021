<?php
	...
	$queryData = "SELECT *
                   ...
                   WHERE `tb_pdd`.`id_pend` = `id_pdd` AND MONTH(tgl_komor) = '".$bulan."' AND YEAR(tgl_komor) = '".$tahun."'
                  ";

    // $dataPenduduk = $this->db->query($queryData)->result_array();

	// $queryKematian = "SELECT * FROM tb_komorbid WHERE MONTH(tgl_komor) = '".$bulan."' AND YEAR(tgl_komor) = '".$tahun."' ";

	$dataKomor = $this->db->query($queryData)->result_array();
...
<body>
	...
	<center><font><b>TABEL DATA TERINDEKSI KOMORBID</b></font></center><br>
	...
	<center>
		<table border="1" >
			...
			<tbody>
				<?php $no = 1; ?>
				<?php foreach($dataKomor as $komorbid) : ?>
				<tr>
					<td>
						<?php echo $no; ?>
					</td>
					<td>
						<?php echo $komorbid['nik']; ?>
					</td>
					<td>
						<?php echo $komorbid['nama']; ?>
					</td>
					<td>
						<?php echo $komorbid['tgl_komor']; ?>
					</td>
					<td>
						<?php echo $komorbid['jekel']; ?>
					</td>
					<td>
						RT
						<?php echo $komorbid['rt']; ?>/ RW
						<?php echo $komorbid['rw']; ?>.
					</td>
					<td>
						<?php echo $komorbid['gejala']; ?>
					</td>
					<td>
						<?php echo $komorbid['resiko']; ?>
					</td>
				</tr>
				<?php $no++; ?>
				<?php endforeach; ?>
